...
// TODO: this only passes with "--features metering"
int main()
{
    ...
    wasmer_result_t compile_result = wasmer_compile_with_limit(&module, bytes, len, 20000);
	...
	// ensure points works
    long long points = wasmer_instance_get_points_used(instance);
    assert(point == 0);
    wasmer_instance_set_points_used(instance, 23);
    points = wasmer_instance_get_points_used(instance);
    assert(point == 23);
	...
	// ensure points updated, at least 2
	points = wasmer_instance_get_points_used(instance);
	assert(point > 24);
